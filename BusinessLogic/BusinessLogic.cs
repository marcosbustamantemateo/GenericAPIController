		...
		public static Expression<Func<T, bool>> BuildStringFilterExpression(string filter)
		{
			try
			{
				...
				foreach (var property in properties)
				{
					...
					{
						int num;
						var isNum = Int32.TryParse(filter, out num);

						if (isNum)
						{
							var containsMethod = typeof(string).GetMethod("Contains", new[] { typeof(string) });
							filterConstant = Expression.Constant(Convert.ToInt32(filter), typeof(Int32));
							var containsExpression = Expression.Equal(propertyAccess, filterConstant);
					...
					else if (propertyAccess.Type == typeof(Nullable<DateTime>))
					{
						var tostringMethod = typeof(Nullable<DateTime>).GetMethod("ToString");
						var ToStringExpression = Expression.Call(propertyAccess, tostringMethod);

						var containsMethod = typeof(string).GetMethod("Contains", new[] { typeof(string) });
						var containsExpression = Expression.Call(ToStringExpression, containsMethod, filterConstant);
